...
Redis.prototype.connect = function (callback) {
  return new Promise(function (resolve, reject) {
    ...
    var _this = this;
    console.log('in Redis.prototype.connect ++++++++++++++');
    this.connector.connect(function (err, stream) {
      if (err) {
        _this.flushQueue(err);
        _this.silentEmit('error', err);
        reject(err);
        console.log('error in this.connector.connect ----------');
        return;
      }
 ...
 * @public
 */
Redis.prototype.reconnect = function (callback) {
  return new Promise(function (resolve, reject) {
    //if (this.status === 'connecting' || this.status === 'connect' || this.status === 'ready') {
    //  reject(new Error('Redis is already connecting/connected'));
    //  return;
    ...
    var _this = this;
    this.connector.reconnect(this.connector, function (err, stream) {
      if (err) {
        _this.flushQueue(err);
        _this.silentEmit('error', err);
        console.log('in this.connector.connect');
        reject(err);
        return;
      }
      var CONNECT_EVENT = _this.options.tls ? 'secureConnect' : 'connect';
